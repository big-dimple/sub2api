...
> 说明：本 fork 的公开 LDAP 版以 `main` 分支的 Docker/源码部署为准。下方二进制安装脚本仍然指向官方 upstream release，不包含本 fork 的 LDAP 定制。LDAP 运维只看 [`deploy/README_LDAP_ENTERPRISE.md`](deploy/README_LDAP_ENTERPRISE.md)。
...
本 fork 的 LDAP Docker/源码部署已禁用网页在线更新。标准升级方式只有：
