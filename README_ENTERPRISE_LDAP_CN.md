...
## 11. 给 IT 的交付口径（可直接转发）
- 当前版本已支持企业 LDAP/AD 接入，运行模式为：本地仅保留 `admin`，员工统一走域账号登录。
- LDAP 上线优先目标是“稳定可控”，先灰度后全量，不引入本地普通账号和人工特例白名单。
- 运维基线：
  - 每日同步一次（`1440` 分钟）保障离职账号失效。
  - 生产禁用“跳过 TLS 校验”。
  - 保留本地 `admin` 作为域控异常时的应急入口。
- 升级策略：
  - 该项目为 fork 维护，但按第 10 节流程定期同步官方 `upstream/main`，避免与开源主线脱节。
  - 企业特有策略留在 fork，通用能力可向 upstream 提 PR。
